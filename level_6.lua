...
function scene:show( event ) 
    ...
    elseif ( phase == "did" ) then 
        ...
        --------------------------  
        local mirrorSet = {
           { -75, 30, 0,"images/wall.png", 20, 200 },
        }
    ...
    -- Static mirror
        ...
        local mirror_d2 = display.newImageRect( sceneGroup, "images/mirror.png", 20, 80 )
        physics.addBody( mirror_d2, "static", { shape={-9,-39,9,-39,9,39,-9,39} } )
        mirror_d2.x = display.contentCenterX + 105
        mirror_d2.y = display.contentCenterY + 100
        mirror_d2.rotation = 90

        
        ------------------------
        -- add paddle
        ------------------------
        local paddle = display.newImageRect( sceneGroup,"images/box.png", 40, 40 )
        paddle.x = display.contentCenterX-45
        paddle.y = display.contentCenterY +20
        physics.addBody( paddle, "static", { density=1, friction=0.3, bounce=0.7 } )
        
        local paddle2 = display.newImageRect( sceneGroup,"images/box.png", 40, 40 )
        paddle2.x = display.contentCenterX+130 
        paddle2.y = display.contentCenterY
        physics.addBody( paddle2, "static", { density=1, friction=0.3, bounce=0.7 } )
        ...
        local function onColl2(x)
                  audio.play( hit )
                  local Forest = movieclip.newAnim({"images/explode1.png","images/explode2.png","images/explode3.png","images/explode4.png","images/explode5.png","images/explode6.png","images/explode7.png","images/explode8.png","images/explode9.png"})
                  Forest:play({startFrame=1,endFrame=9,loop=1,remove=true})
                  Forest.x,Forest.y = paddle2.x,paddle2.y 
                  Forest.width,Forest.height=150,150
                  Forest:setDrag{drag=true}
                  paddle2:removeSelf()
                  paddle2 = nil
        ...
        -- add turrent
        ------------------------
        local turret = display.newImageRect( sceneGroup,"images/turret.png", 40, 47 )
        physics.addBody( turret, "static", { radius=15 } )
        turret.x = display.contentCenterX+95
        turret.y = display.contentCenterY-185
        turret.rotation = -160
        
        local turret2 = display.newImageRect( sceneGroup,"images/turret.png", 40, 47 )
        physics.addBody( turret2, "static", { radius=15 } )
        turret2.x = display.contentCenterX-135
        turret2.y = display.contentCenterY-25
        turret2.rotation = 45
        ...
        local function castRay( startX, startY, endX, endY )
          ...
          if ( hits and beamGroup.numChildren <= 50 ) then
            ...
            if ( hitFirst.object == paddle2) then
             --paddle2 break
             onColl2()
            end
       ...
       local function fireOnTimer( event )
         ...
         if ( beamGroup.numChildren == 0 ) then
           ...
           local xDest = turret2.x - (math.cos(math.rad(turret2.rotation+90)) * 1600 )
           local yDest = turret2.y - (math.sin(math.rad(turret2.rotation+90)) * 1600 )
           -- Cast the initial ray
           castRay( turret2.x, turret2.y, xDest, yDest )
         end
       end
      ...
      local start_b = display.newText( sceneGroup,"Start", turret.x, turret.y-1, native.systemFont, 16 )
      start_b:setFillColor( 1, 1, 1)
      turret:addEventListener( "tap", fireOnTimer ) 
      
      local start_b2 = display.newText( sceneGroup,"Start", turret2.x, turret2.y+5, native.systemFont, 16 )
      start_b2:setFillColor( 1, 1, 1) 
      turret2:addEventListener( "tap", fireOnTimer ) 
      ...
      local function run()
         if(switch_1==2) then
            ...
            turret2:removeEventListener( "tap", fireOnTimer ) 
            ...
            local function clear()       
               composer.gotoScene("m_end",{ time=500, effect="fade" } )                        
            end      
